...
class Data:
    ...
    def create_data(self) -> pd.DataFrame:
        ...
        #modif pas très classe
        count_ru = pd.DataFrame(mov_rat['userId'].value_counts())
        count_ru = count_ru[count_ru['count'] > 4000].reset_index()

        mov_rat = pd.merge(mov_rat,count_ru,on = 'userId',how='inner')
        mov_rat = mov_rat[mov_rat['count']>4000]

        gen_sco_tag[gen_sco_tag['relevance']>0.8]

        df = pd.merge(mov_rat,gen_sco_tag).drop_duplicates(subset=['title','userId'])
